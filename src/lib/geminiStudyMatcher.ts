...
const geminiApiKey = env.EXPO_PUBLIC_GEMINI_API_KEY;
...
type MatchStudiesInput = {
  patientQuery: string;
  profile?: StudyPulseProfile | null;
...
function buildStudyMatchingPrompt(
  patientQuery: string,
  profile: StudyPulseProfile | null | undefined,
  studies: StudyProgram[]
) {
...
function scoreStudyLocally(
  ...
  return {
    caution:
      cautions[0] ?? (status === 'likely_fit' ? 'No major issues flagged.' : 'Review details with the clinic.'),
    reason:
